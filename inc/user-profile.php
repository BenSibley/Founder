...
function ct_founder_user_profile_image_setting( $user ) {
...
add_action( 'show_user_profile', 'ct_founder_user_profile_image_setting' );
add_action( 'edit_user_profile', 'ct_founder_user_profile_image_setting' );

/**
 * Saves additional user fields to the database
 */
function ct_founder_save_user_profile_image( $user_id ) {

	// only saves if the current user can edit current user profile
	if ( ! current_user_can( 'edit_user', $user_id ) ) return false;

	update_user_meta( $user_id, 'founder_user_profile_image', esc_url_raw( $_POST['founder_user_profile_image'] ) );
}

add_action( 'personal_options_update', 'ct_founder_save_user_profile_image' );
add_action( 'edit_user_profile_update', 'ct_founder_save_user_profile_image' );

// add the social profile boxes to the user screen.
function ct_founder_add_social_profile_settings($user) {
	...
	<table class="form-table">
		<tr>
			<th><h3><?php _e('Social Profiles', 'founder'); ?></h3></th>
		</tr>
		<?php
		foreach($social_sites as $key => $social_site) { ?>
			<tr>
				<th>
					<?php if( $key == 'email' ) : ?>
						<label for="<?php echo $key; ?>-profile"><?php echo ucfirst($key); ?> <?php _e('Address:', 'founder'); ?></label>
					<?php else : ?>
						<label for="<?php echo $key; ?>-profile"><?php echo ucfirst($key); ?> <?php _e('Profile:', 'founder'); ?></label>
					<?php endif; ?>
				</th>
				<td>
					<?php if( $key == 'email' ) : ?>
...
add_action( 'show_user_profile', 'ct_founder_add_social_profile_settings' );
add_action( 'edit_user_profile', 'ct_founder_add_social_profile_settings' );

function ct_founder_save_social_profiles($user_id) {
...
add_action( 'personal_options_update', 'ct_founder_save_social_profiles' );
add_action( 'edit_user_profile_update', 'ct_founder_save_social_profiles' );
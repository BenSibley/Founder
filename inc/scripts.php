...
function ct_founder_load_scripts_styles() {
	...
	if( is_rtl() ) {
		wp_enqueue_style('ct-founder-style-rtl', get_template_directory_uri() . '/styles/rtl.min.css');
	} else {
		wp_enqueue_style('ct-founder-style', get_template_directory_uri() . '/style.min.css');
	}
	...
	wp_script_add_data( 'ct-founder-html5-shiv', 'conditional', 'if IE 8' );
	...
	wp_script_add_data( 'ct-founder-respond', 'conditional', 'if IE 8' );
}
 ...
 */
function founder_enqueue_customizer_post_message_scripts(){
...
add_action('customize_preview_init','founder_enqueue_customizer_post_message_scripts');
